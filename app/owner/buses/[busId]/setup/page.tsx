...
export default function BusSetupPage({ params }: { params: Promise<{ busId: string }> }) {
    ...
    // Apply reverse trip stops from a specific trip
    const applyReverseTrip = (sourceTripIndex: number) => {
        const sourceTrip = tripConfigs[sourceTripIndex];
        if (!sourceTrip.stops.some(s => s.name.trim() !== '')) {
            setError('Selected trip has no stops configured');
            return;
        }
